...
/* USER CODE BEGIN PFP */
void vTask1(void *pvtaskhandler);
void vTask2(void *pvtaskhandler);
/* USER CODE END PFP */
...
int main(void)
{
 ...
 // osThreadDef(defaultTask, StartDefaultTask, osPriorityNormal, 0, 128);
  ...
  uint8_t status;
  status = xTaskCreate (vTask1, "task1",1000, NULL, 2,NULL);
  configASSERT(status == pdPASS);
  status = xTaskCreate(vTask2, "task2", 1000, NULL, 1, NULL);
  configASSERT(status == pdPASS);
  /* add threads, ... */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	//  HAL_UART_Transmit(&huart3, "working\n\r",9 , 0xFFFFFF);
  }
  /* USER CODE END 3 */
}
...

{
	...
	//char *pcTaskName;
	const TickType_t xdelay1000ms = pdMS_TO_TICKS(2000);
   while(1)
   {
	  // HAL_UART_Transmit(&huart3, "task1\n\r",9 , 0xFFFFFF);
	//   HAL_GPIO(GPIOB, LD1_Pin);
...

{
		...
		while(1)
   {
	   ...
	   vTaskDelayUntil(&xPrevwaketime, pdMS_TO_TICKS(1000));
	   	//   vTaskDelay(xdelay1000ms);
